...
df = pd.read_parquet('data/daily/ts_code=000001.SZ/data.parquet')
...
cutoff = (
    pd.to_datetime(ind['trade_date'].max(), format='%Y%m%d') - pd.DateOffset(months=6)
).strftime('%Y%m%d')
...
print('近 6 个月 MACD 金叉（截止', ind['trade_date'].max(), '）')
